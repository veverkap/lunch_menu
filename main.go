...
func sendTelegramImage(photoData []byte) error {
	...
	// 1. Create a bytes.Buffer to store the multipart body
	body := &bytes.Buffer{}
	// 2. Create a multipart.Writer
	writer := multipart.NewWriter(body)

	// 3a. Create a form file field and write the byte array to it
	part, err := writer.CreateFormFile("photo", "menu_image.png")
	if err != nil {
		log.Fatal(err)
	}
	// Write the byte array data to the form file part
	...
	// 4. Don't forget to close the multipart writer to add the terminating boundary
	err = writer.Close()
	if err != nil {
		log.Fatal(err)
	}
	...
	// 6. Set the Content-Type header to the multipart writer's Content-Type,
	// which includes the necessary boundary information.
...
func getTomorrowsWeather(date time.Time) (string, error) {
	body, err := os.ReadFile("weather.txt")
	if err != nil {
		logger.Error("Failed to read weather response from file", "error", err)
		return "", err
	}
